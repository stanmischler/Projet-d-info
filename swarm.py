...
class swarm:
    def __init__(self, S, d, environment):
        ...
        self.best_global_points=self.paths[best_index].copy().points
    ...
    def generate_next(self, current_path, w, c_1, c_2):
    ...
    def swarm_update(self,w,c_1,c_2):
        new_paths=[self.generate_next(p,w,c_1,c_2) for p in self.paths]
        fitnesses=[p.fitness() for p in new_paths]
        best_index=fitnesses.index(max(fitnesses))

        if fitnesses[best_index]>self.best_global_fitness:
            self.best_global_points=new_paths[best_index].copy().points
            self.best_global_fitness=fitnesses[best_index]
